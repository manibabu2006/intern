...
import { deleteOTP } from "./_otpstore.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (dbOTP !== otp.trim()) return res.status(400).json({ success: false, message: "Invalid OTP" });

    if (new Date(rows[0].expires_at) < new Date()) {
      await deleteOTP(username.trim(), cleanRole);
      return res.status(400).json({ success: false, message: "OTP expired" });
    }

    const hashed = await bcrypt.hash(newPassword, 10);
    await db.execute(`UPDATE ${table} SET password = ? WHERE username = ?`, [
      hashed,
      username.trim(),
    ]);
